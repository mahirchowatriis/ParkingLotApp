                ...
                droneLocationLiveData.observe(activity, Observer { currentLocation ->
                    ...
                    when {
                        ...
                        !travelledLongitude -> {//!travelledLongitude
                            ...
                            chooseDirection(
                                longitudeDiff,
                                Direction(pitch = (mission.autoFlightSpeed * (abs(longitudeDiff) / originalLongitudeDiff)).toFloat()),
                                Direction(pitch = (mission.autoFlightSpeed * -1 * (abs(longitudeDiff) / originalLongitudeDiff)).toFloat())
                            )
                        }
                        ...
                        travelledLongitude -> {//travelledLongitude
                            ...
                            chooseDirection(
                                latitudeDiff,
                                Direction(roll = (mission.autoFlightSpeed * (abs(latitudeDiff) / originalLatitudeDiff)).toFloat()),
                                Direction(roll = (mission.autoFlightSpeed * -1 * (abs(latitudeDiff) / originalLatitudeDiff)).toFloat())
                            )
                        }
                    }
